...
const Dashboard = () => {
  ...
  // Keep the stored time zone current (first visit, or after travelling) so the
  // calendar feed shows meals at the times set in Account settings.
  useEffect(() => {
    if (!user || !profile) return;
    const tz = Intl.DateTimeFormat().resolvedOptions().timeZone;
    if (tz && (profile as { timezone?: string | null }).timezone !== tz) {
      void supabase.from("profiles").update({ timezone: tz } as never).eq("user_id", user.id);
    }
  }, [user, profile]);
